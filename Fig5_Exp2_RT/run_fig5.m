...
% plot model
fig_handle = draw_plot_exp2(1, D.Choice(I), [D.sColCoh1(I), abs(D.sColCoh2(I))], D.RT(I), [], modelo, 0, colors);

% plot data
subplot(2,2,1);hold all; title({'Unknown color (fits)',''});
for c = 1:6
...
draw_plot_exp2(2, D.Choice, [D.sColCoh1, D.sColCoh2], D.RT, D.Cond, [modelo1,modelo2],0, red);

% plot data - choices
subplot(2,1,1);hold all;
for c = 2:-1:1
    if c == 1
        errorbar(unique(cohDiff),nanmean(choice_suj_cond(:,c,:),3), nanstd(choice_suj_cond(:,c,:),[],3)/sqrt(length(IDs)),'o','Color',red,'MarkerSize',7,'LineWidth',1.5,'MarkerFaceColor',[1 1 1]);
    else
        errorbar(unique(cohDiff),nanmean(choice_suj_cond(:,c,:),3), nanstd(choice_suj_cond(:,c,:),[],3)/sqrt(length(IDs)),'.','Color',red,'MarkerSize',26,'LineWidth',1.5);
    end
end
% plot data - RTs
...
for c = 2:-1:1
    if c == 1
        errorbar(unique(cohDiff),nanmean(rt_suj_cond(:,c,:),3), nanstd(rt_suj_cond(:,c,:),[],3)/sqrt(length(IDs)),'o','Color',red,'MarkerSize',7,'LineWidth',1.5,'MarkerFaceColor',[1 1 1]);
    else
        errorbar(unique(cohDiff),nanmean(rt_suj_cond(:,c,:),3), nanstd(rt_suj_cond(:,c,:),[],3)/sqrt(length(IDs)),'.','Color',red,'MarkerSize',26,'LineWidth',1.5);
    end
end
